...
exports.deploySafeContracts = deploySafeContracts;
...
async function deploySafeContracts() {
    ...
    const wallet = new ethers_1.Wallet(deployerPrivateKey, provider);
    ...
    const safeSingletonFactory = new ethers_1.ethers.ContractFactory([], bytecodes.SafeSingleton, wallet);
    ...
    const proxyFactoryFactory = new ethers_1.ethers.ContractFactory([], bytecodes.SafeProxyFactory, wallet);
    ...
    const multiSendFactory = new ethers_1.ethers.ContractFactory([], bytecodes.MultiSend || bytecodes.SafeSingleton, wallet);
    ...
    const createCallFactory = new ethers_1.ethers.ContractFactory([], bytecodes.CreateCall || bytecodes.SafeSingleton, wallet);
    ...
    const safeSingletonInterface = new ethers_1.ethers.Interface([
    ...
    // Deploy Safe proxy
    const proxyFactoryContract = new ethers_1.Contract(proxyFactoryAddress, ['function createProxy(address _singleton, bytes memory initializer) public returns (address proxy)'], wallet);
    ...
    const proxyCreationEvent = receipt.logs.find((log) => {
        try {
            const iface = new ethers_1.ethers.Interface(['event ProxyCreation(address indexed proxy, address singleton)']);
    ...
    const deployment = {
        ...
        testSafe: proxyCreationEvent ?
            new ethers_1.ethers.Interface(['event ProxyCreation(address indexed proxy, address singleton)']).parseLog(proxyCreationEvent)?.args.proxy :
            'deployment-failed',
