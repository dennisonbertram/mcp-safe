...
export class NetworkProviderManager {
    ...
    async getProvider(networkId, runtimeConfig) {
        // Check if network is configured
        if (!this.isNetworkSupported(networkId)) {
            throw new SafeError(`Network ${networkId} is not configured`, 'NETWORK_NOT_CONFIGURED');
        }
    ...
    getProviderHealth(networkId) {
        return this.healthStatus.get(networkId);
    }
    markProviderUnhealthy(networkId, error) {
