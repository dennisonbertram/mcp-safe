...
export const safeDeployInfrastructure = {
    ...
    async handle(input, networkManager) {
        ...
        try {
            ...
            if (balance === 0n) {
                throw new SafeError('Deployer account has no balance', 'INSUFFICIENT_BALANCE', { deployer: deployer.address, balance: ethers.formatEther(balance) });
            }
            ...
            const deployment = await deploySafeInfrastructure(provider, deployer, gasPrice, confirmations);
            return {
                success: true,
                networkId: network,
            ...
            throw new SafeError(`Failed to deploy Safe infrastructure: ${error instanceof Error ? error.message : String(error)}`, 'DEPLOYMENT_FAILED', { network, error: error instanceof Error ? error.stack : String(error) });
        }
    },
};
/**
...
async function deploySafeInfrastructure(provider, deployer, gasPrice, confirmations = 1) {
    ...
    const safeSingletonResult = await deploySafeSingleton(provider, deployer, singletonFactoryResult.address, gasOptions, confirmations);
    ...
    const proxyFactoryResult = await deploySafeProxyFactory(provider, deployer, singletonFactoryResult.address, gasOptions, confirmations);
    ...
    const fallbackHandlerResult = await deployFallbackHandler(provider, deployer, singletonFactoryResult.address, gasOptions, confirmations);
    ...
    const multiSendResult = await deployMultiSend(provider, deployer, singletonFactoryResult.address, gasOptions, confirmations);
 ...
 */
async function deploySingletonFactory(provider, deployer, gasOptions, confirmations) {
...
async function deploySafeSingleton(provider, deployer, singletonFactoryAddress, gasOptions, confirmations) {
    ...
    const address = ethers.getCreate2Address(singletonFactoryAddress, salt, ethers.keccak256(bytecode));
...
function getSingletonFactoryBytecode() {
...
function getSafeSingletonBytecode() {
    ...
    try {
        ...
        throw new SafeError('Failed to load real Safe artifacts. Run: node scripts/get-safe-artifacts.js', 'ARTIFACTS_NOT_FOUND', { error: error instanceof Error ? error.message : String(error) });
    }
}
function getSafeProxyFactoryBytecode() {
...
function getFallbackHandlerBytecode() {
