...
export class RealWalletQueryTools {
    ...
    async handleGetSafeInfo(arguments_) {
        ...
        try {
            ...
            throw new SafeError(`Failed to get Safe info: ${error instanceof Error ? error.message : String(error)}`, ErrorCodes.SAFE_OPERATION_ERROR);
        }
    }
    async getSafeInfo(address, networkId, rpcUrl) {
        try {
            ...
            const [owners, threshold, nonce, version, modules, guard, fallbackHandler,] = await Promise.all([
                safeSdk.getOwners(),
            ...
            throw new SafeError(`Failed to retrieve Safe information: ${error instanceof Error ? error.message : String(error)}`, ErrorCodes.SAFE_OPERATION_ERROR);
        }
    }
    async getGuard(safeSdk) {
        try {
    ...
    async getFallbackHandler(safeSdk) {
        try {
