...
describe('WalletCreationTools', () => {
    ...
    describe('safe_create_wallet_config tool', () => {
        ...
        it('should validate valid wallet configuration', async () => {
            ...
            const configResult = JSON.parse(result.content[0]?.text);
        ...
        it('should reject invalid owner addresses', async () => {
            ...
            const configResult = JSON.parse(result.content[0]?.text);
            expect(configResult.isValid).toBe(false);
            expect(configResult.errors.some((err) => err.includes('Invalid owner address'))).toBe(true);
        });
        ...
        it('should provide validation warnings for security considerations', async () => {
            ...
            expect(configResult.warnings.some((warning) => warning.includes('Single owner'))).toBe(true);
        });
        it('should handle missing required parameters', async () => {
            ...
            const configResult = JSON.parse(result.content[0]?.text);
            expect(configResult.isValid).toBe(false);
            expect(configResult.errors.some((err) => err.includes('threshold'))).toBe(true);
            expect(configResult.errors.some((err) => err.includes('networkId'))).toBe(true);
        });
    ...
    describe('safe_predict_address tool', () => {
        ...
        it('should predict address for valid wallet configuration', async () => {
            ...
            expect(result.isError).toBe(false);
            expect(result.content[0]?.type).toBe('text');
            const addressResult = JSON.parse(result.content[0]?.text);
        ...
        it('should reject invalid configuration for prediction', async () => {
            ...
            expect(result.isError).toBe(true);
            const errorResult = JSON.parse(result.content[0]?.text);
    ...
    describe('safe_deploy_wallet tool', () => {
        ...
        it('should require private key for deployment', async () => {
            ...
            expect(result.isError).toBe(true);
            expect(result.content[0]?.text).toContain('Private key is required');
        });
