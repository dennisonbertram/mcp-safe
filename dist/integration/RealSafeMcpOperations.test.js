...
import { TestAccountManager } from '../test/TestAccountManager.js';
...
describe('Real Safe MCP Operations Integration', () => {
    ...
    let walletCreationTools;
    let walletQueryTools;
    let ownerManagementTools;
    let transactionManagementTools;
    let testAccounts;
    beforeAll(async () => {
    ...
    describe('Wallet Creation Tools with Real Contracts', () => {
        ...
        test('safe_deploy_wallet - should deploy real Safe wallet', async () => {
            ...
            if (response.isError)
                return;
            const deployment = JSON.parse(response.content[0]?.text || '{}');
            ...
            global.testSafeAddress = deployment.address;
        }, 15000);
    });
    describe('Wallet Query Tools with Real Contracts', () => {
        ...
        beforeAll(async () => {
            ...
            safeAddress = global.testSafeAddress;
            if (!safeAddress) {
        ...
        test('safe_get_info - should retrieve real Safe information', async () => {
            ...
            if (response.isError)
                return;
            const safeInfo = JSON.parse(response.content[0]?.text || '{}');
    ...
    describe('Owner Management Tools with Real Contracts', () => {
        ...
        test('safe_add_owner - should add new owner to real Safe', async () => {
            ...
            const safeInfo = JSON.parse(infoResponse.content[0]?.text || '{}');
    ...
    describe('Transaction Management Tools with Real Contracts', () => {
        let transactionSafeAddress;
        let transactionOwnerKeys;
        beforeAll(async () => {
        ...
        test('safe_propose_transaction - should propose transaction on real Safe', async () => {
            ...
            global.testTransactionHash = result.safeTransactionHash;
        }, 20000);
    ...
    describe('Real Contract Compatibility Verification', () => {
        test('should verify Safe contract ABI compatibility', async () => {
            ...
            const safeSingleton = await testUtils.getContractAt('MockSafeSingleton', deployment.contracts.safeSingleton);
        ...
        test('should verify proxy factory functionality', async () => {
            const deployment = testUtils.getDeploymentInfo();
            const factory = await testUtils.getContractAt('MockSafeProxyFactory', deployment.contracts.safeProxyFactory);
        ...
        test('should verify Safe proxy delegation works', async () => {
            ...
            const safe = await testUtils.getContractAt('MockSafeSingleton', deployment.address);
    ...
    describe('Performance and Gas Usage', () => {
        ...
        test('should track gas usage for transactions', async () => {
            ...
            const gasUsage = await testUtils.executeSafeTransactionWithGasTracking(deployment.address, testAccounts[1].address, '1.0', '0x', [testAccounts[0].privateKey]);
