...
describe('SafeProtocolKit', () => {
    ...
    describe('createSafeInstance', () => {
        it('should create Safe instance with valid parameters', async () => {
            ...
            const safeInstance = await safeKit.createSafeInstance(safeAddress, networkId);
        ...
        it('should throw error for invalid Safe address', async () => {
            ...
            const networkId = 'eip155:1';
            await expect(safeKit.createSafeInstance(invalidAddress, networkId)).rejects.toThrow('Invalid Safe address');
        });
        it('should throw error for unsupported network', async () => {
            const safeAddress = '0x1234567890123456789012345678901234567890';
            const networkId = 'eip155:999';
            await expect(safeKit.createSafeInstance(safeAddress, networkId)).rejects.toThrow('Network eip155:999 is not supported');
        });
        it('should cache Safe instances for reuse', async () => {
            const safeAddress = '0x1234567890123456789012345678901234567890';
            const networkId = 'eip155:1';
            const instance1 = await safeKit.createSafeInstance(safeAddress, networkId);
            const instance2 = await safeKit.createSafeInstance(safeAddress, networkId);
    ...
    describe('deploySafe', () => {
        ...
        it('should throw error for invalid threshold', async () => {
            ...
            const networkId = 'eip155:1';
            await expect(safeKit.deploySafe({
                owners,
                threshold,
                networkId,
    ...
    describe('getSafeInfo', () => {
        ...
        it('should throw error for non-existent Safe', async () => {
            ...
            const networkId = 'eip155:1';
            await expect(safeKit.getSafeInfo(safeAddress, networkId)).rejects.toThrow('Safe not found or not deployed');
        });
    });
    describe('createTransaction', () => {
        it.skip('should create transaction with valid parameters', async () => {
            ...
            const transaction = await safeKit.createTransaction(safeAddress, networkId, txData);
        ...
        it.skip('should support batch transactions', async () => {
            ...
            const batchTransaction = await safeKit.createBatchTransaction(safeAddress, networkId, transactions);
        ...
        it.skip('should estimate gas correctly', async () => {
            ...
            const gasEstimate = await safeKit.estimateTransactionGas(safeAddress, networkId, txData);
    ...
    describe('signTransaction', () => {
        it.skip('should sign transaction with private key', async () => {
            ...
            const transaction = await safeKit.createTransaction(safeAddress, networkId, txData);
            const signature = await safeKit.signTransaction(transaction, privateKey, networkId);
        ...
        it('should validate signer is Safe owner', async () => {
            const safeAddress = '0x1234567890123456789012345678901234567890';
            const networkId = 'eip155:1';
            const invalidPrivateKey = '0x5678901234567890123456789012345678901234567890123456789012345678';
            const txData = {
                to: '0x2345678901234567890123456789012345678901',
                value: '1000000000000000000',
                data: '0x',
            ...
            const transaction = await safeKit.createTransaction(safeAddress, networkId, txData);
            await expect(safeKit.signTransaction(transaction, invalidPrivateKey, networkId)).rejects.toThrow('Signer is not an owner of this Safe');
        });
    });
    describe('executeTransaction', () => {
        it.skip('should execute transaction when threshold is met', async () => {
            const safeAddress = '0x1234567890123456789012345678901234567890';
            const networkId = 'eip155:1';
            const executorPrivateKey = '0x4f3edf983ac636a65a842ce7c78d9aa706d3b113bce9c46f30d7d21715b23b1d';
            const txData = {
                to: '0x2345678901234567890123456789012345678901',
                value: '1000000000000000000',
                data: '0x',
            ...
            const executionResult = await safeKit.executeTransaction(transaction, signatures, executorPrivateKey, networkId);
        ...
        it('should throw error when threshold not met', async () => {
            ...
            const transaction = await safeKit.createTransaction(safeAddress, networkId, txData);
            const signatures = []; // Empty signatures
            await expect(safeKit.executeTransaction(transaction, signatures, executorPrivateKey, networkId)).rejects.toThrow('Insufficient signatures to meet threshold');
        });
    });
    describe('getBalance', () => {
        ...
        it.skip('should get token balance', async () => {
            ...
            const networkId = 'eip155:1';
            const balance = await safeKit.getTokenBalance(safeAddress, tokenAddress, networkId);
            expect(balance).toBeDefined();
            expect(typeof balance).toBe('string');
    ...
    describe('Transaction State Management', () => {
        it.skip('should cache transaction state', async () => {
            ...
            const transaction1 = await safeKit.createTransaction(safeAddress, networkId, txData);
            const transaction2 = await safeKit.createTransaction(safeAddress, networkId, txData);
