...
export class NetworkManager {
   ...
   */
  async getProvider(networkId: string): Promise<JsonRpcProvider> {
    // Check cache first
    ...
    if (!networkConfig) {
      throw new SafeError(
        ...
        { networkId, supportedNetworks: Array.from(this.networkConfigs.keys()) }
      );
    }
   ...
   */
  parseNetworkId(networkId: string): { namespace: string; chainId: number } {
    const parts = networkId.split(':');
    if (parts.length !== 2 || parts[0] !== 'eip155') {
      throw new SafeError(
        `Invalid network ID format: ${networkId}. Expected format: eip155:chainId`,
    ...
    if (isNaN(chainId)) {
      throw new SafeError(
        `Invalid chain ID in network ID: ${networkId}`,
        'INVALID_CHAIN_ID',
        { networkId }
      );
    }
   ...
   */
  async testNetwork(networkId: string): Promise<{
    success: boolean;
    latency?: number;
    blockNumber?: number;
    error?: string;
