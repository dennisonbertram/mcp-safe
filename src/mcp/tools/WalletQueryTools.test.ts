...
describe('WalletQueryTools', () => {
  ...
  describe('safe_get_info tool', () => {
    it('should be registered as an MCP tool', () => {
      ...
      expect(infoTool?.description).toContain(
        'Get comprehensive information about a Safe wallet'
      );
    });
    ...
    it('should get info for valid Safe address', async () => {
      ...
      const result = await queryTools.handleToolCall(
        'safe_get_info',
        validQuery
      );
    ...
    it('should require networkId parameter', async () => {
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as any)?.text).toContain(
        'Required field missing: networkId'
      );
    });
