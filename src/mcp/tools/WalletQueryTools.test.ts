...
describe('WalletQueryTools', () => {
  let queryTools: WalletQueryTools;
  ...
  describe('safe_get_info tool', () => {
    ...
    it('should get info for valid Safe address', async () => {
      ...
      const safeInfo = JSON.parse((result.content[0] as any)?.text as string);
    ...
    it('should reject unsupported networks', async () => {
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as any)?.text).toContain('Network eip155:999 is not supported');
    });
    ...
    it('should require address parameter', async () => {
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as any)?.text).toContain('Required field missing: address');
    });
    ...
    it('should require networkId parameter', async () => {
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as any)?.text).toContain('Required field missing: networkId');
    });
    ...
    it('should return consistent data for same address and network', async () => {
      ...
      const safeInfo1 = JSON.parse((result1.content[0] as any)?.text as string);
      const safeInfo2 = JSON.parse((result2.content[0] as any)?.text as string);
  ...
  describe('MCP Tool Interface', () => {
    ...
    it('should handle tool execution errors gracefully', async () => {
      ...
      expect(result.isError).toBe(true);
      expect((result.content[0] as any)?.text).toContain('Invalid input');
    });
  });
});