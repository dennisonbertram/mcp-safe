...
describe('WalletCreationTools', () => {
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockImplementation((networkId: string) => {
  ...
  describe('safe_create_wallet_config tool', () => {
    it('should be registered as an MCP tool', () => {
      ...
      const configTool = tools.find(
        (tool) => tool.name === 'safe_create_wallet_config'
      );

      expect(configTool).toBeDefined();
      expect(configTool?.description).toContain(
        'Validate and configure Safe wallet parameters'
      );
    });
    ...
    it('should validate valid wallet configuration', async () => {
      ...
      const result = await walletTools.handleToolCall(
    ...
    it('should reject invalid owner addresses', async () => {
      ...
      expect(configResult.isValid).toBe(false);
      expect(
        configResult.errors.some((err: string) =>
          err.includes('Invalid owner address')
        )
      ).toBe(true);
    });
    ...
    it('should reject unsupported networks', async () => {
      ...
      expect(configResult.isValid).toBe(false);
      expect(configResult.errors).toContain(
        'Network eip155:999 is not supported'
      );
    });
    ...
    it('should validate fallback handler addresses', async () => {
      ...
      expect(configResult.isValid).toBe(true);
      expect(configResult.configuration.fallbackHandler).toBe(
        configWithHandler.fallbackHandler
      );
    });
    ...
    it('should validate module and guard configurations', async () => {
      ...
      expect(configResult.isValid).toBe(true);
      expect(configResult.configuration.modules).toEqual(
        configWithModules.modules
      );
    ...
    it('should provide validation warnings for security considerations', async () => {
      ...
      expect(
        configResult.warnings.some((warning: string) =>
    ...
    it('should handle missing required parameters', async () => {
      ...
      expect(configResult.isValid).toBe(false);
      expect(
        configResult.errors.some((err: string) => err.includes('threshold'))
      ).toBe(true);
      expect(
        configResult.errors.some((err: string) => err.includes('networkId'))
      ).toBe(true);
    });
  ...
  describe('safe_predict_address tool', () => {
    it('should be registered as an MCP tool', () => {
      const tools = walletTools.getTools();
      const predictTool = tools.find(
        (tool) => tool.name === 'safe_predict_address'
      );

      expect(predictTool).toBeDefined();
      expect(predictTool?.description).toContain(
        'Predict the address of a Safe wallet'
      );
    });
    ...
    it('should predict address for valid wallet configuration', async () => {
      ...
      const addressResult = JSON.parse(
        (result.content[0] as any)?.text as string
      );
  ...
  describe('safe_deploy_wallet tool', () => {
    it('should be registered as an MCP tool', () => {
      const tools = walletTools.getTools();
      const deployTool = tools.find(
        (tool) => tool.name === 'safe_deploy_wallet'
      );
    ...
    it('should handle deployment with optional parameters', async () => {
      ...
      expect(deployResult.address).toMatch(/^0x[a-fA-F0-9]{40}$/);
      expect(deployResult.configuration.fallbackHandler).toBe(
        configWithOptions.fallbackHandler
      );
      expect(deployResult.configuration.modules).toEqual(
        configWithOptions.modules
      );
    });
