...
describe('SafeApiService', () => {
  ...
  describe('proposeTransaction', () => {
    it('should propose transaction to Safe Transaction Service', async () => {
      ...
      const proposalResult = await apiService.proposeTransaction(
        safeAddress,
        networkId,
        transaction,
        senderSignature
  ...
  describe('getPendingTransactions', () => {
    it('should retrieve pending transactions for Safe', async () => {
      ...
      const pendingTxs = await apiService.getPendingTransactions(
        safeAddress,
        networkId
      );
  ...
  describe('getTransactionHistory', () => {
    ...
    it('should support filtering by executed status', async () => {
      ...
      const executedTxs = await apiService.getTransactionHistory(
        safeAddress,
        networkId,
        { executed: true }
      );
  ...
  describe('confirmTransaction', () => {
    it('should add confirmation to pending transaction', async () => {
      ...
      const confirmationResult = await apiService.confirmTransaction(
        safeTxHash,
        networkId,
        signature
      );
    ...
    it('should throw error for invalid transaction hash', async () => {
      ...
      await expect(
        apiService.confirmTransaction(safeTxHash, networkId, signature)
  ...
  describe('getTransaction', () => {
    it('should retrieve specific transaction by hash', async () => {
      ...
      const transaction = await apiService.getTransaction(
        safeTxHash,
        networkId
      );
    ...
    it('should throw error for non-existent transaction', async () => {
      ...
      await expect(
        apiService.getTransaction(safeTxHash, networkId)
  ...
  describe('getServiceUrl', () => {
    ...
    it('should throw error for unsupported network', () => {
      expect(() => apiService.getServiceUrl('eip155:999')).toThrow(
        'Safe Transaction Service not available for network eip155:999'
      );
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle service unavailability gracefully', async () => {
      ...
      await expect(
        apiService.getPendingTransactions(safeAddress, networkId)
    ...
    it('should retry failed requests', async () => {
      ...
      const result = await (apiService as any).withRetry(testOperation);
