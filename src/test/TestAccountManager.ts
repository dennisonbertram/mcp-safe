...
export class TestAccountManager {
  ...
  async getTestAccounts(count: number = 10): Promise<TestAccount[]> {
    if (this.accounts.length === 0) {
      await this.loadHardhatAccounts(count);
    }
   ...
   */
  async getDeployerAccount(): Promise<TestAccount> {
    return this.getTestAccount(0);
  }

  /**
   * Create a wallet instance for a test account
   */
  async createWallet(accountIndex: number): Promise<Wallet> {
    const account = await this.getTestAccount(accountIndex);
    return new Wallet(account.privateKey, this.provider);
  }
  ...
  async fundAccount(
    ...
    const fromAccount = await this.getTestAccount(fromIndex);
    const wallet = new Wallet(fromAccount.privateKey, this.provider);
   ...
   */
  private async loadHardhatAccounts(count: number): Promise<void> {
    // Hardhat's deterministic private keys (first 20)
    ...
    for (let i = 0; i < Math.min(count, hardhatPrivateKeys.length); i++) {
      const privateKey = hardhatPrivateKeys[i];
  ...
  async createMultiSigScenario(
    ...
    return {
      owners,
      threshold,
      ownerAddresses: owners.map((account) => account.address),
      ownerPrivateKeys: owners.map((account) => account.privateKey),
    };
  }
